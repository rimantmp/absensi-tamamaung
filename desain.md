...
description: Sistem Absensi Siswa Berbasis Barcode adalah aplikasi website untuk mencatat kehadiran siswa di SD Inpres Tamamaung IV menggunakan barcode/QR code. Sistem ini membantu sekolah mencatat absensi secara lebih cepat, rapi, dan mudah direkap dibandingkan pencatatan manual. Sistem hanya berfokus pada absensi siswa. Sistem tidak mencakup notifikasi orang tua, WhatsApp gateway, pembayaran, akademik, nilai, atau fitur administrasi sekolah lain di luar kebutuhan absensi.
...
| `{typography.nav-link}` | 14px | 500 | 1.4 | 0 | Sidebar/sidenav menu items |
...
| Level | Treatment | Use |
|---|---|---|
| Flat | No shadow, no border | Body sections, public scan header, hero |
...
### Internal Sidebar / Sidenav

**`app-shell`** — Internal authenticated pages use a two-column dashboard shell: a persistent left sidebar/sidenav and a main content canvas. Do not use a top navigation bar for authenticated admin/operator, guru, kepala sekolah, or siswa-private pages.

**`sidebar-nav`** — Left sidenav with `{colors.surface-soft}` background, 1px right border using `{colors.hairline}`, fixed desktop width around 260px, and `{spacing.lg}` padding. It carries the school/app name, current user identity/role, role-based menu items, link to public scan if needed, and logout action.

**`sidebar-nav-item`** / **`sidebar-nav-item-active`** — Vertical menu items in `{typography.nav-link}`. Inactive items are transparent; active items use `{colors.surface-card}` background, `{colors.ink}` text, and `{rounded.md}` radius.

**`public-scan-shell`** — The public student self-attendance page is the only page that may omit the internal sidebar. It should show a simple header/action area and the scan interface immediately. It must not expose internal navigation, reports, history, or other students' data.
...
|---|---|---|
| Mobile | < 768px | Sidebar collapses into a drawer or stacked mobile menu; hero h1 72→36px; hero-illustration-card stacks below; feature grids 1-up; pricing 1-up |
| Tablet | 768–1024px | Sidebar can collapse to compact width or drawer; feature cards 2-up; pricing 2-up |
| Desktop | 1024–1440px | Persistent left sidebar/sidenav; 3-up feature cards; 3-up pricing tiers |
...
### Collapsing Strategy
- Internal sidebar/sidenav collapses to a drawer or stacked mobile menu at < 768px.
- Do not introduce a top navigation bar for authenticated app pages. Keep role-based navigation in the sidebar/sidenav.
- Public student self-attendance may use a minimal header because it is outside the authenticated internal app shell.
